...
{
	public class DundasChartBuilder : Chart, ChartBuilder
	{
		...
		public DundasChartBuilder(ChartSettings cs)
		{
			chartSettings = cs;
		}
		...
		public void init(IEnumerable<SeriesData> seriesData)
		{
			...
			if (chartSettings.ShowLegend)
			{
				...
				Legends.Add(new Legend()
				{
					...
					Name = legendName,
					DockInsideChartArea = true,
					DockToChartArea = ca.Name,
		...
		public Series CreateSeries(SeriesData sd, int currentSeriesNumber, int totalNumberOfSeries, string legendName)
		{
			...
			Series series = new Series()
			{
				...
				Type = GetSeriesChartType(sd.GetChartType()),
				//ChartType = chartType,
			...
			if (!string.IsNullOrEmpty(sd.GetPieLabelStyle()))
				series.SetAttribute("PieLabelStyle", sd.GetPieLabelStyle());
			...
			if (seriesCount == 1)
			{
				...
				if (totalNumberOfSeries == 1)
					chartArea.AxisY.LabelStyle.FontColor = Color.Black;
				else
		...
		public ChartArea CreateYAxis(ChartArea area, Series series, Color axisColor, Color axisLineColor, string axisName, float axisOffset, float labelsSize)
		{
			...
			if (axisColor != null)
			{
				...
				axis.LineWidth = 3;
				axis.LabelStyle.FontColor = axisColor;
		...
		public ChartArea CreateY2Axis(ChartArea area, Series series, Color axisColor, Color axisLineColor, string axisName, float axisOffset, float labelsSize)
		{
			...
			areaSeries.AxisY2.StartFromZero = area.AxisY2.StartFromZero;
			...
			*/
			...
			areaAxis.AxisY2.StartFromZero = area.AxisY2.StartFromZero;
		...
		public void SetStripLines(Axis axis, List<StripLineSettings> slsList)
		{
			if (slsList != null && slsList.Count() > 0)
			{
				foreach (StripLineSettings sls in slsList)
				{
					...
					int offset = axis.StripLines.Add(new StripLine());
					...
					if(!string.IsNullOrEmpty(sls.HREF))
						axis.StripLines[offset].Href = sls.HREF;
				}
			}
		}
		...
		{
			chartArea.AlignOrientation = AreaAlignOrientation.Horizontal;

			//chartArea.AxisY.Interval = 100;
			//chartArea.AxisY.IntervalAutoMode = IntervalAutoMode.VariableCount;
			Dundas.Charting.WebControl.Axis axis = chartArea.AxisX;
			...
			axis.LabelsAutoFitStyle = LabelsAutoFitStyle.DecreaseFont;
			...
			Dundas.Charting.WebControl.Label label = new Dundas.Charting.WebControl.Label();
			...
			{
				BackGradientEndColor = chartBackgrouundColorEnd;
				chartArea.BackGradientType = GradientType.TopBottom;
			}
		...
		protected void setChartBackgroundBlank()
		{
			...
			BackGradientType = GradientType.None;
		...
		*/
		...
		protected Dundas.Charting.WebControl.Label GetLabelWithFont(Font font, Color fontColor)
		{
			if (fontColor == null)
				fontColor = Color.Black;

			if (font == null)
				font = new Font(GetDefaultFontName(), 10, FontStyle.Regular);

			Dundas.Charting.WebControl.Label label = new Dundas.Charting.WebControl.Label();
			//label.ShowEndLabels = true;
			label.TruncatedLabels = false;
			label.OffsetLabels = false;
		...
		protected Dundas.Charting.WebControl.Label GetLabel(Color fontColor)
		{
			return GetLabelWithFont(null, fontColor);
		}
		protected Dundas.Charting.WebControl.Label GetLabelWithSize(Color fontColor, int fontSize)
		{
			Font font = new Font(GetDefaultFontName(), fontSize > 0 ? fontSize : 10, FontStyle.Regular);
			return GetLabelWithFont(font, fontColor);
		}

		static protected void SetScaleBreak(AxisScale axis)
		{
			// Enable scale breaks
			axis.ScaleBreakStyle.Enabled = true;

			// Set the scale break type
			axis.ScaleBreakStyle.BreakLineType = BreakLineType.Wave;
			...
			axis.ScaleBreakStyle.StartFromZero = AutoBool.True;// AutoBool.Auto;
		}

		static private void addCustomLabels(Axis axis)
		{
			// Set X axis custom labels
			int element = axis.CustomLabels.Add(0, 30, "Low");
			element = axis.CustomLabels.Add(30, 70, "Medium");
			element = axis.CustomLabels.Add(70, 100, "High");
		...
		public Annotation CreateAnnotation(DateTime date, string content, Guid partId, string partName)
		{
			...
			annotation.LineStyle = ChartDashStyle.Solid;
		...
		public static void ChartVerticalXAxis_PrePaint(object sender, ChartPaintEventArgs e)
		{
			if (sender is ChartArea)
			{
				...
				for (int index = 0; index < series.Points.Count; index++)
				{
					...
					SizeF labelSize = e.ChartGraphics.MeasureString(axisLabel,
		...
		public void CreateWaterMark(Chart Chart1, System.Drawing.Image img, Single alpha, System.Drawing.Color color, int width, int height, Dundas.Charting.WebControl.ChartPaintEventArgs e)
		{
		...
		public static System.Drawing.Imaging.ImageAttributes getImageAttributes()
		{
			System.Drawing.Imaging.ImageAttributes attrib = new System.Drawing.Imaging.ImageAttributes();
		...
		{
			DundasChartBuilder chart = e.Chart as DundasChartBuilder;
			if (chart == null)
				return;
			...
			if (img != null)
			{
				...
				Rectangle innerRectangle = new Rectangle((int)(e.Chart.Width.Value - img.Width), (int)0, img.Width, img.Height);
				//The object is used to apply changes to the image
				...
				//matrix[3, 3] = 256 - alpha;
				//attrib.SetColorMatrix(matrix);
				e.ChartGraphics.DrawImage(img, innerRectangle, 0, 0, img.Width, img.Height, System.Drawing.GraphicsUnit.Pixel, null); //, attrib);
				// Use label's size and position information to define "hot" areas
				if (!string.IsNullOrEmpty(helpText) || !string.IsNullOrEmpty(helpHREF))
				{
					// From Dundas
					//MapArea ma = new MapArea();
					//float[] coords = { 0f, 0f, 10f, 10f };
					//ma.Coordinates = coords;
					//ma.Shape = MapAreaShape.Rectangle;
					//ma.Href = helpHREF;
					//ma.ToolTip = chart.chartSettings.HelpIconText;
