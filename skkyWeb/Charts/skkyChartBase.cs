...
{
	public class skkyChartBase : Chart
	{
		...
		private skkyChartBase()
		{ }

		public skkyChartBase(ChartSettings cs)
		{
			chartSettings = cs;
		}
		...
		public void init(IEnumerable<SeriesData> seriesData)
		{
			...
			ChartArea ca = CreateChartArea(seriesData.First().Settings, seriesData.Count());
		...
		public void SetStripLines(Axis axis, List<StripLineSettings> slsList)
		{
			if (slsList != null && slsList.Count() > 0)
			{
				foreach (StripLineSettings sls in slsList)
				{
					...
					{
						axis.StripLines[offset].Url = "javascript:void(0);";
						axis.StripLines[offset].MapAreaAttributes = "onclick=\"" + sls.HREF + "\"";
					}
				}
			}
		}
		...
		protected ChartArea CreateChartArea(SeriesSettings firstSeries, int seriesCount)
		{
			...
			if (ca != null)
			{
				...
				if (seriesCount > 0)
				{
					...
					if (SeriesSettings.Const_BarChart == firstSeries.ChartType || SeriesSettings.Const_ColumnChart == firstSeries.ChartType)
					{
						UpdateChartAreaForBar(ca
							, firstSeries.XAxis.Title
							, firstSeries.YAxis.Title
							, backBegin
							, backEnd);
					}
					else if (firstSeries.ChartType == SeriesSettings.Const_SplineChart)
					{
						setChartBackgroundEmbossed();
						if(null != firstSeries)
						{
							if (null != firstSeries.XAxis && !string.IsNullOrWhiteSpace(firstSeries.XAxis.Title))
							{
								Axis xaxis = ca.AxisX;
								xaxis.Title = firstSeries.XAxis.Title;
								xaxis.TitleFont = GetXAxisTitleFont(xaxis.TitleFont, Width.Value, Height.Value);
							}

							if (null != firstSeries.YAxis && !string.IsNullOrWhiteSpace(firstSeries.YAxis.Title))
							{
								Axis yaxis = ca.AxisY;
								yaxis.Title = firstSeries.YAxis.Title;
								yaxis.TitleFont = GetYAxisTitleFont(yaxis.TitleFont, Width.Value, Height.Value);
							}
						}
					}

					//if (Property.IsDoubleType(firstSeries.YAxis.FieldDescription.PropertyType))
...
/*
	...
	*/		
		...
		*/
		...
		public virtual int populateSeries(Series series, SeriesData sd, bool enableHref, bool enableToolTips)
		{
			int count = 0;
			if (series != null && sd != null)
			{
				for (int i = 0; i < sd.RowCount(); ++i)
				{
					var soa = sd.getPoint(i);
					if (null == soa || null == soa.objValue)
						continue;

					int offset = series.Points.AddXY(soa.stringValue, soa.objValue);

					DataPoint p = series.Points[offset];

					DataPointSettingsWithObjects dps = sd.GetDataPointSettings(i);
					if (dps != null)
					{
					...
					if(enableToolTips)
						p.ToolTip = sd.GetToolTip(i);
					if (enableHref)
					{
						if (sd.Settings.HREFIsJavascript)
						{
							p.Url = "javascript:void(0);";
							p.MapAreaAttributes = "onclick=\"" + sd.GetHREF(i) + "\"";
						}
						else
						{
							p.Url = sd.GetHREF(i);
					...
					++count;
					if (sd.GetMaxPointsToShow() > 1 && count >= sd.GetMaxPointsToShow())
						break;
				}
			}
		...
		{
			skkyChartBase chart = e.Chart as skkyChartBase;
			if (chart == null)
				return;
