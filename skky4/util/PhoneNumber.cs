...
namespace skky.util
{
    ...
    public struct PhoneNumber : IComparable, IFormattable, ISerializable
    {
		...
		public static string Normalize(string country, string phone)
		{
			...
			if (!string.IsNullOrEmpty(str))
			{
				if (country.Equals("ireland"))
				{
					...
					if (str.Length >= 10 && str.StartsWith("353"))
						str.Mid(3);
				...
				else if (country.Equals("united states"))
				{
					...
					//Regex.Replace(phone, "(\d{3})\D*(\d{3})\D*(\d{4})\D*(\d*)$", str);
					try
					{
						PhoneNumber pn = new PhoneNumber(str, true);
						str = pn.ToString();
//						if (!string.IsNullOrEmpty(str))
//							str = "+1 " + str;
					}
